...
df_numeric$condition <- factor(df_numeric$condition, levels = c("NR", "R"))
...
ggplot(df_numeric %>% filter(question == "Q1"),
  ...
  labs(
    x = "Condition", y = "Rating", title = "RQ1: Ratings per Condition", fill="Condition") 
...
ggplot(df_numeric %>% filter(question == "Q2"),
  ...
  geom_boxplot(width = 0.1, fill = "white") +
  facet_grid(cols = vars(type)) +
  labs(x = "Condition", y = "Rating", title = "RQ2: Ratings per Condition", fill="Condition") 
```
...
saveRDS(fit_Q1, file = "~/fit_Q1.rds")
summary(fit_Q1)
...
```{r}
saveRDS(fit_Q1, file = "~/fit_Q2.rds")
summary(fit_Q2)
```

```{r}
#check deltas (stefano throws me in jail version)

df_diff$diff_num = as.numeric(df_diff$diff)

fit_delta_lme <- lmer(diff_num ~ condition * type +
                   (1|ResponseId) +
                   (1|item),
                    data = df_diff)
```
...
```{r}
summary(fit_delta_lme)
```



```{r}
# check deltas (stefano does not throw me in jail version?)

df2 <- df_diff %>%
  pivot_longer(cols = c(Q1, Q2), names_to = "question", values_to = "rating") %>%
  mutate(rating = ordered(rating), question = factor(question))

fit_delta <- brm(
  rating ~ condition * question * type + (1 | ResponseId) + (1 | item),
  data = df2,
  family = cumulative()
)
...
saveRDS(fit_delta, file = "~/fit_delta.rds")
summary(fit_delta)
```
